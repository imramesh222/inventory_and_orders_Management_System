...
def create_item(req: CreateItemRequest, db: Session = Depends(get_db)):
    ...
    return CreateItemResponse(item=item, error=False, code=200, msg="Item created successfully")
...
def get_item(item_id: str, db: Session = Depends(get_db)):
    ...
    return GetItemResponse(item=item, error=False, code=200, msg="Item fetched successfully")
...
def update_item(item_id: str, req: UpdateItemRequest, db: Session = Depends(get_db)):
    ...
    if not updated_item:
        raise HTTPException(status_code=404, detail="Item not found")
    return UpdateItemResponse(item=updated_item, error=False, code=200, msg="Item updated successfully")

@router.get("/", response_model=ListItemResponse)
def list_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    service = ItemService(db)
    items = service.list_items(skip=skip, limit=limit)
    return ListItemResponse(items=items, error=False, code=200, msg="Items listed successfully")
