...
def create_order(req: CreateOrderRequest, db: Session = Depends(get_db)):
    ...
    return CreateOrderResponse(order=order, error=False, code=ErrorCode.SUCCESS, msg="Order created successfully")
...
def get_order(order_id: str, db: Session = Depends(get_db)):
    ...
    return GetOrderResponse(order=order, error=False, code=ErrorCode.SUCCESS, msg="Order fetched successfully")
...
def update_order(order_id: str, req: UpdateOrderRequest, db: Session = Depends(get_db)):
    ...
    if not updated_order:
        raise HTTPException(status_code=404, detail="Order not found")
    return UpdateOrderResponse(order=updated_order, error=False, code=ErrorCode.SUCCESS, msg="Order updated successfully")

@router.get("/", response_model=ListOrderResponse)
def list_orders(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    service = OrderService(db)
    orders = service.list_orders(skip=skip, limit=limit)
    return ListOrderResponse(orders=orders, error=False, code=ErrorCode.SUCCESS, msg="Orders listed successfully")